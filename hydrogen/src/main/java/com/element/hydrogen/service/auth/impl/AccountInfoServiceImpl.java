...
public class AccountInfoServiceImpl implements AccountInfoService {
     ...
     */
    @Override
    public ResponseJson updatePwd(DonAccountInfoEntity donAccountInfoEntity) {
        ResponseJson resJson = new ResponseJson();
        try {
            DonAccountInfoEntity entity = donAccountInfoMapper.queryByUserId(donAccountInfoEntity);
            if (entity != null ){
                if (entity.getPwd().equals(donAccountInfoEntity.getPwd())){
                    resJson.setCode(ResponseJsonConstant.FAIL);
                    resJson.setMsg("新密码不能与旧密码相同");
                }else {
                    donAccountInfoMapper.updatePwd(donAccountInfoEntity);
                    resJson.setCode(ResponseJsonConstant.SUCCESS);
                    resJson.setMsg(ResponseJsonConstant.SUC_UPD);
                }
            }else {
                resJson.setCode(ResponseJsonConstant.FAIL);
                resJson.setMsg("账号不存在");
            }
        } catch (Exception e) {
            resJson.setCode(ResponseJsonConstant.FAIL);
            resJson.setMsg(ResponseJsonConstant.FAIL_UPD);
